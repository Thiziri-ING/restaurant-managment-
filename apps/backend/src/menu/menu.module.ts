...
export class MenuItemsService {
  ...
  async findAll(dto: {
    ...
    // Fix : comparaison stricte boolean pour éviter le 400
    if (dto.available === true) {
      where.available = true;
    } else if (dto.available === false) {
      where.available = false;
    }

    const limit = dto.limit ?? 20;
    const skip = getSkip(dto);
...
export class MenuItemsController {
  ...
  findAll(
    ...
    // Conversion manuelle string → boolean pour éviter le 400
