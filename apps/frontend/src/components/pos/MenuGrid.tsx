    ...
    <div className="flex h-full flex-col gap-3">
      ...
      {isLoading ? (
        ...
        <div className="grid flex-1 grid-cols-2 gap-3 overflow-y-auto sm:grid-cols-3">
          {items.data.map((item) => {
            // Conversion explicite Decimal → number pour éviter l'erreur toFixed
            ...
            return (
              <button
                key={item.id}
                onClick={() =>
                  addItem({ id: item.id, name: item.name, price })
                }
              ...
              >
                ...
                <span className="mt-1 text-sm font-bold text-primary-600">
                  {price.toFixed(2)} DA
                </span>
              </button>
            );
